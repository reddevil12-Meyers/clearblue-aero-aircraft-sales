...
export default function ClientAircraftTab({ clientId }) {
  ...
  useEffect(() => {
    base44.entities.Aircraft.filter({ seller_id: clientId })
      .then(setAircraft).finally(() => setLoading(false));
  }, [clientId]);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-4">
        <p className="text-sm text-muted-foreground">{aircraft.length} aircraft linked to this client</p>
        <Link to="/aircraft/new">
          <Button size="sm" variant="outline" className="gap-2"><Plus className="w-4 h-4" />Add Aircraft</Button>
        </Link>
      </div>
      {aircraft.length === 0 ? (
        ...
        <div className="space-y-2">
          {aircraft.map(ac => (
            <Link key={ac.id} to={`/aircraft/${ac.id}`} className="flex items-center justify-between p-4 bg-card border border-border rounded-lg hover:bg-muted/50 transition-colors">
