...
export default function StepReport({ form, update, appraisalId, onSave }) {
  ...
  useEffect(() => {
      ...
      promises.push(
        supabase.from('valuation_runs').select('*').eq('appraisal_id', appraisalId).then(({ data }) => {
          ...
          if (runs.length > 0) {
            ...
            setRun(latest);
            return supabase.from('valuation_adjustments').select('*').eq('valuation_run_id', latest.id).then(({ data: adj }) => setAdjustments(adj || []));
          }
        })
      );
    }
    if (form.aircraft_id) {
      promises.push(
        supabase.from('comps').select('*').eq('aircraft_id', form.aircraft_id).then(({ data }) => setComps(data || []))
      );
      promises.push(
        supabase.from('aircraft').select('*').then(({ data }) => {
          const ac = (data || []).find(a => a.id === form.aircraft_id);
          setAircraft(ac || null);
        })
      );
    }
    if (form.client_id) {
      promises.push(
        supabase.from('clients').select('*').then(({ data }) => {
          const cl = (data || []).find(c => c.id === form.client_id);
          setClient(cl || null);
        })
      );
    }
  ...
  const handleGenerateNarratives = async () => {
    setGenerating(true);
    toast({ title: "AI feature coming soon", description: "AI narrative generation will be available shortly." });
    setGenerating(false);
  };
